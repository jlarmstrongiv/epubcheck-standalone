...
public class TProperties extends THashtable<Object, Object> {
    ...
    private synchronized void load(StreamOrReader sr) throws IOException {
        ...
        while (true) {
            ...
            // Stock code masked with 0xff, truncating every char above U+00FF
            // when loading through a Reader (load(Reader) must preserve full
            // chars; the byte-stream path already yields 0..255 only). This
            // silently corrupted non-Latin-1 chars in .properties bundles
            // (e.g. U+2019 in epubcheck messages became U+0019).
