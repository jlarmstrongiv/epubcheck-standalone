...
public class TThrowable extends RuntimeException {
    ...
    private void init(TThrowable cause) {
        ...
        this.writableStackTrace = true;
        fillInStackTrace();
        // --- epubcheck-standalone TeaVM shim fix: the JDK's Throwable(Throwable)
        // constructor sets detailMessage to cause.toString(); stock TeaVM
        // leaves it null, so `new RuntimeException(e)` -- e.g. epubcheck's
        // EpubCheckFactory wrapping a URL-input failure -- printed a bare
        // "java.lang.RuntimeException" headline where the jar prints
        // "java.lang.RuntimeException: <cause.toString()>". Jar-parity
        // failure output depends on the JDK behavior.
