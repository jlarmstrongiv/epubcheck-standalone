...
public class TRandomAccessFile implements DataInput, DataOutput, Closeable {
    ...
    // When the opened path IS the host-backed book (ecshim.HostBook), serve it
    // via long-offset host range reads instead of a VFS accessor: the accessor
    // API is int-sized end to end (seek/tell/size), which caps it at 2 GiB --
    // a ZIP64 multi-GB book needs real long positions (ZipFile seeks past
    // 4 GiB for the central directory). `hostOpen` is this mode's open flag;
    // `accessor` stays null. The same interception serves host-backed
    // expanded-directory files (ecshim.HostDir, zero-byte VFS placeholders
    // whose bytes the host serves per file): `hostDirPath` selects the
    // HostDir per-file feed (null means the single-book HostBook feed).
    private boolean hostOpen;
    private long hostPos;
    private String hostDirPath;

    private int shimHostRead(byte[] b, int off, int len) throws IOException {
        return hostDirPath != null
                ? ecshim.HostDir.read(hostDirPath, hostPos, b, off, len)
                : ecshim.HostBook.read(hostPos, b, off, len);
    }

    private long shimHostSize() {
    ...
    public TRandomAccessFile(TFile file, String mode) throws FileNotFoundException {
        ...
        if (readOnly && file.shimIsHostBook()) {
            hostOpen = true;
            buff = new byte[16];
            return;
        }

        if (readOnly && file.shimIsHostDirFile()) {
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        ensureOpened();
        if (hostOpen) {
            int result = shimHostRead(b, off, len);
            if (result > 0) {
                hostPos += result;
            }
            return result;
    ...
    public int read() throws IOException {
        ...
        int read = hostOpen ? readHost(buffer) : accessor.read(buffer, 0, 1);
        // epubcheck-standalone shim fix: stock code returned the SIGNED byte; the
        // read() contract is an unsigned 0..255 value (bytes >= 0x80 came back
        // negative and were mistaken for EOF, e.g. in zip EOCD scanning).
        return read > 0 ? buffer[0] & 0xff : -1;
    }

    private int readHost(byte[] buffer) throws IOException {
        int n = shimHostRead(buffer, 0, 1);
        if (n > 0) {
            hostPos += n;
        }
        return n;
    ...
    public int skipBytes(int n) throws IOException {
        ...
        if (hostOpen) {
            ...
            long remaining = shimHostSize() - hostPos;
            long skipped = Math.min((long) n, Math.max(0L, remaining));
            hostPos += skipped;
            return (int) skipped;
        }
    ...
    public void seek(long pos) throws IOException {
        ensureOpened();
        if (hostOpen) {
            // Real long positions: the host-backed book seeks past 4 GiB
            // (ZIP64 central directories); no int truncation here.
            hostPos = pos;
            return;
        }
    ...
    public void setLength(long newLength) throws IOException {
        ensureOpened();
        if (hostOpen) {
            throw new IOException("host-backed book is read-only");
        }
