...
public class TFile implements Serializable, Comparable<TFile> {
    ...
    // --- epubcheck-standalone TeaVM shim addition: host-backed book interception ---
    // The packaged epub is served by the JS host through range reads
    // (ecshim.HostBook) instead of being materialized in the in-memory VFS
    // (whose whole API is int-sized -- a multi-GB book cannot exist there).
    // This ONE path reports as an existing, readable, host-sized file; every
    // other path stays on the VFS. TFileInputStream and TRandomAccessFile
    // route the same path to HostBook for content reads.
    boolean shimIsHostBook() {
        return ecshim.HostBook.matches(fs().canonicalize(getCanonicalPathImpl()));
    }

    // Host-backed EXPANDED-DIRECTORY file (ecshim.HostDir): the file exists in
    // the VFS as a zero-byte placeholder (so listing/exists/isFile work through
    // the stock machinery), but its size and content are served by the JS host
    // on demand. Only length() needs routing here; TFileInputStream and
    // TRandomAccessFile route the same canonical path for content reads.
    boolean shimIsHostDirFile() {
        return ecshim.HostDir.matches(fs().canonicalize(getCanonicalPathImpl()));
    }

    String shimCanonicalPath() {
    ...
    public long length() {
        if (shimIsHostBook()) {
            // The host-backed book's REAL size: the VFS VirtualFile.length()
            // is int-typed and could not represent a multi-GB book anyway.
            return ecshim.HostBook.size();
        }
        if (shimIsHostDirFile()) {
            // A host-backed expanded-directory file: the VFS placeholder is
            // zero bytes; the listed size comes from the host feed.
            return ecshim.HostDir.sizeOf(shimCanonicalPath());
        }
        VirtualFile virtualFile = findVirtualFile();
    ...
    public org.teavm.classlib.java.nio.file.TPath toPath() {
        return org.teavm.classlib.java.nio.file.TPaths.get(getPath());
    }
