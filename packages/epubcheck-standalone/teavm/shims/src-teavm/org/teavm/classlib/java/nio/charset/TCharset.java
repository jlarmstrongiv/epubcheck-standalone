...
public abstract class TCharset implements Comparable<TCharset> {
    ...
    public static TCharset forName(String charsetName) {
        ...
        // epubcheck-standalone shim fix (fork of the stock classlib file): translate
        // the JDK-proprietary Unicode* aliases (used by epubcheck's
        // XMLEncodingSniffer) onto the canonical UTF-16 charsets.
        String lookupName = charsetName;
        String lower = charsetName.toLowerCase();
        switch (lower) {
            case "unicodebig":
        ...
        TCharset charset = Charsets.value.get(lookupName.toUpperCase());
        if (charset == null) {
