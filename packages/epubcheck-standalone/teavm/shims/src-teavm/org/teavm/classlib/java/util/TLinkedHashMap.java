...
public class TLinkedHashMap<K, V> extends THashMap<K, V> implements TSequencedMap<K, V> {
    ...
    public V getOrDefault(Object key, V defaultValue) {
        ...
        if (key == null) {
            ...
            int hash = THashMap.spreadHash(key);
    ...
    V putImpl(K key, V value, boolean first, boolean forceMotion) {
        ...
        // epubcheck-standalone shim: same spread as the THashMap fork so entries
        // stored here are found by the inherited THashMap lookups.
        int hash = key == null ? 0 : THashMap.spreadHash(key);
