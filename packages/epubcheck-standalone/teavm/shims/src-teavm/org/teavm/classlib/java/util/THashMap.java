...
public class THashMap<K, V> extends TAbstractMap<K, V> implements TCloneable, TSerializable {
    ...
    // JDK-faithful iteration order: (1) spread the hash like java.util.HashMap
    // (h ^ h>>>16) before indexing, (2) append new entries at the bucket tail
    // (Harmony prepended), (3) preserve chain order across rehash (Harmony
    // reversed). Report output (e.g. epubcheck's items array, which is sorted
    // with a stable sort keyed on non-unique ids) leaks HashMap iteration
    // order; matching the JVM makes those outputs byte-identical.
    ...
    private HashEntry<K, V> createHashedEntry(K key, int index, int hash) {
        ...
        // epubcheck-standalone shim: append at bucket tail (JDK order)
        HashEntry<K, V> cur = elementData[index];
        if (cur == null) {
    ...
    void rehash(int capacity) {
        ...
        HashEntry<K, V>[] tails = newElementArray(length);
        for (int i = 0; i < elementData.length; i++) {
            HashEntry<K, V> entry = elementData[i];
            elementData[i] = null;
            while (entry != null) {
                ...
                // epubcheck-standalone shim: append to preserve chain order (JDK)
                entry.next = null;
                if (newData[index] == null) {
                    newData[index] = entry;
                } else {
                    tails[index].next = entry;
                }
                tails[index] = entry;
                entry = next;
            }
