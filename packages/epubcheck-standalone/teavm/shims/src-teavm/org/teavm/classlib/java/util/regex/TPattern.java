            ...
            switch (lexemes.peek()) {
                ...
                case TLexer.CHAR_HYPHEN: {
                    if (firstInClass || lexemes.lookAhead() == TLexer.CHAR_RIGHT_SQUARE_BRACKET
                        ...
                        // JDK accepts '^' as the literal END of a range inside a class, e.g.
                        // [+-^] (epubcheck's MIME-type regex uses exactly this). The lexer
                        // tokenizes '^' as special CHAR_CARET even mid-class, so the stock
                        // code below threw PatternSyntaxException. Treat it as literal '^'.
                        if (cur == TLexer.CHAR_CARET) {
                            try {
                                res.add(buffer, '^');
                            } catch (Exception e) {
