...
public class TDefaultFileSystemProvider extends TFileSystemProvider {
    ...
    public TFileSystem getFileSystem(TURI uri) {
        // epubcheck-standalone shim fix (fork of the stock classlib file): the stock
        // code required path == "/", which makes Paths.get(URI) fail for every
        // real file URI (TPath.of routes through getFileSystem). Also tolerate
        // the empty (non-null) authority TeaVM's URI parser produces for
        // file:/// URIs.
    ...
    public TPath getPath(TURI uri) {
        // epubcheck-standalone shim fix (fork of the stock classlib file): TeaVM's URI
        // parser reports an EMPTY (non-null) authority for file:/// URIs where
        // the JDK reports null; accept both.
        if (!uri.getScheme().equals("file") || (uri.getAuthority() != null && !uri.getAuthority().isEmpty())
    ...
    public InputStream newInputStream(TPath path, TOpenOption... options) throws IOException {
        ...
        // epubcheck-standalone shim: open through the forked
        // TFileInputStream(TFile) entry point (not a raw VFS accessor) so a
        // HOST-BACKED path -- the range-fed book (ecshim.HostBook) or an
        // expanded-directory file (ecshim.HostDir, a zero-byte VFS
        // placeholder whose content streams from the host) -- serves its real
        // bytes to java.nio readers too. epubcheck's direct-directory
        // validation (OCFDirectoryResources) reads container files via
        // Files.newInputStream, which previously hit the empty placeholder.
        // Non-host paths take the identical accessor path inside the fork.
        try {
            return new TFileInputStream(new org.teavm.classlib.java.io.TFile(
    ...
    public <A extends TBasicFileAttributes> A readAttributes(TPath path, Class<A> type, TLinkOption... options) throws
        ...
        return (A) new TBasicFileAttributes() {
            @Override
            public long size() {
                // epubcheck-standalone shim: report host-backed sizes (the
                // range-fed book, expanded-directory placeholder files)
                // through the forked TFile.length(), which routes them to the
                // host feed; a plain VFS file gets its virtual length as
                // before. epubcheck's direct-directory validation takes item
                // sizes from these attributes.
                return new org.teavm.classlib.java.io.TFile(defaultPath.pathString).length();
            }
