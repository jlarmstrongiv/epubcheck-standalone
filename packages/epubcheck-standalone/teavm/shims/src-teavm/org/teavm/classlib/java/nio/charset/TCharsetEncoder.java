...
public abstract class TCharsetEncoder {
    ...
    protected TCharsetEncoder(TCharset cs, float averageBytesPerChar, float maxBytesPerChar, byte[] replacement) {
        // epubcheck-standalone shim fix (fork of the stock classlib file):
        // assign maxBytesPerChar BEFORE validating the replacement, like the
        // JDK does -- stock code called checkReplacement first, so the check
        // always compared against the uninitialized field (0.0), which is why
        // the inverted length check below never fired at construction time.
        this.charset = cs;
    ...
    private void checkReplacement(byte[] replacement) {
        // epubcheck-standalone shim fix (fork of the stock classlib file): the
        // stock length check is INVERTED vs the JDK. java.nio.charset
        // .CharsetEncoder documents the precondition as "newReplacement is
        // neither null nor empty, its length does not exceed maxBytesPerChar"
        // -- i.e. reject replacement.length > maxBytesPerChar. Stock TeaVM
        // rejected replacement.length < maxBytesPerChar instead, outlawing
        // every SHORT legal replacement (e.g. commons-compress
        // NioZipEncoding's single "?" byte on a UTF-8 encoder, reached by
        // epubcheck's --mode exp zip writer).
